...
class kgyy:
    ...
    async def submit(self,user,taskid,ap=None):
        ...
        param,key = await self.get_param(user,ts)
        data1 = {"taskid":taskid,
                "t2":"a46276d24d1b73efc00b037f9418b5eb5c7e556efc5394826b9a5ca0869f0f7165491fb34d37faee7a583dd6ae62c7fed5065452b75c0f61058574a453d71e9a153e0af5c3d3eb72508cc3ccf9b50f78a990c10421529671f55d64e80c3f1dd19b5b5013e520766f9a36e5d39de56dbf",
                "clienttime_ms": str(ts1),
                "user_label":{"val9":8192,"val8":0,"val7":0,"val6":0,"val5":0,"val4":0,"val3":0,"val2":0,"val1":0}
                }
        if ap:
            data1.update(ap)
        if taskid == 1107:
            ...
            data1.update({"meal_id": 1 if 5< hour <= 9 else 2 if 11<= hour <= 14 else 3 if 17 <= hour <=20 else 4 if 21<= hour < 24 else data1.get("meal_id")})
        data = json.dumps(data1)
        signature = await self.create_sign(key,data)
        ...
        if res['errcode'] == 0:
            if taskid == 1107:
                ...
                if 'double_code' in res['data']:
                    print(f"id:{taskid} 可以加金币,试着加金币,等待十几秒")
    ...
    async def check_env(self):
        ...
        if cks is None:
            print('你没有填写酷狗音乐ck环境变量，请填写：kgyycks 变量')
        else:
            cks_list = cks.split('@')
            correct_data = []
            for cka in cks_list:
                if len(cka.split('#')) == 4:
                    ...
                    print("第{cks_list.index(cka)}账号填写格式不正确，格式:xxxx#openid#nickname#ture=2")
            return correct_data
